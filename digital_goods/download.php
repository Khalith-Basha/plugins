<?php
    ...
    if(preg_mathc('|([0-9]+)_([0-9]+)_(.+)|', $filename, $tmp)) {
        $tmp = explode("_", $filename);

        $conn = getConnection();
        $download = $conn->osc_dbFetchResult("SELECT * FROM %st_item_dg_files WHERE fk_i_item_id = %d AND s_name = '%s' AND s_code = '%s'", DB_TABLE_PREFIX, $tmp[2], $tmp[3], $tmp[1]);
